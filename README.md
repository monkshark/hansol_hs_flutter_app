...
| 지표 | 수치 | 비고 |
|------|------|------|
| **총 코드 라인** | **약 51,900** | Dart 44,349(생성 코드 제외) + TypeScript/TSX 3,057 + JS 2,940 + Java/XML 1,174 + Swift 373 |
| **소스 파일** | **193개** (Flutter, 생성 코드 제외) + **30개** (Admin Web TS/TSX) + Android/iOS 위젯 | screens, 추출 위젯, models/utils/services |
...
| **Dart 라인 수** | **44,349** | `find lib test integration_test -name '*.dart' ! -name '*.g.dart' ! -name '*.freezed.dart' \| xargs cat \| wc -l` |
| **Dart 파일 수** | **193개** | `find lib test integration_test -name '*.dart' ! -name '*.g.dart' ! -name '*.freezed.dart' \| wc -l` |
